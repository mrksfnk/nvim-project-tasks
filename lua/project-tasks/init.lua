...
function M.run_task(task_name, opts)
	...
	if not task then
		vim.notify(
			("[project-tasks] Task '%s' not available for %s"):format(task_name, backend_name),
			vim.log.levels.INFO
		)
		return
	end
	...
	local ctx = {
		...
		env = vim.tbl_extend(
			"force",
			backend.env or {},
			M.config.project and M.config.project.env or {},
			opts.env or {}
		),
		args = opts.args or {},
		variables = vim.tbl_extend(
			"force",
			backend.variables or {},
			M.config.project and M.config.project.variables or {}
		),
	}
